...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        if not self.batch_first:
            raise Error("Input tensor properly")
...
class ConvBLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        y_out_rev, _ = self.reverse_net(x.flip(dims=(2,)))

        y_out_fwd = y_out_fwd[-1] # outputs of last CLSTM layer = B, C, T, H, W
        y_out_rev = y_out_rev[-1] # outputs of last CLSTM layer = B, C, T, H, W

        reversed_idx = list(reversed(range(y_out_rev.shape[2])))
        y_out_rev = y_out_rev[:, :, reversed_idx, ...] # reverse temporal outputs.
        ycat = torch.cat((y_out_fwd, y_out_rev), dim=1)
        
        return ycat # B, C, T, H, W


if __name__ == "__main__":

    x1 = torch.randn([5, 1, 60, 60]).cuda()
    x2 = torch.randn([5, 1, 60, 60]).cuda()
    x3 = torch.randn([5, 1, 60, 60]).cuda()

    cblstm = ConvBLSTM(in_channels=1, hidden_channels=32, kernel_size=(3, 3), num_layers=1, batch_first=True).cuda()
